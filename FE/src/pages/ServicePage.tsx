...
export default function ServicePage() {
  ...
  const handleWeekChange = (direction: 'next' | 'prev') => {
    ...
    setCurrentWeek(prev => {
      ...
      if (direction === 'prev' && isSameWeek(newWeek, startOfThisWeek, { weekStartsOn: 1 })) {
        return startOfThisWeek;
      }
      if (direction === 'next' && isSameWeek(newWeek, startOfNextWeek, { weekStartsOn: 1 })) {
  ...
  return (
    <div className="bg-gradient-to-b from-sky-50 to-[#f0f7fa] min-h-screen flex flex-col">
      ...
      <div className="flex-1 w-full px-4 py-8 flex justify-center items-start">
        {showIntro ? (
          <div className="max-w-7xl w-full mx-auto flex flex-col items-center justify-center min-h-[80vh]">
              ...
              <div className="flex flex-col-reverse md:flex-row items-center gap-12">
                ...
                <div className="w-full md:w-1/2 text-left space-y-8">
                  ...
                  <button
                    ...
                    className="mt-8 px-10 py-4 bg-gradient-to-r from-cyan-500 to-blue-500 text-white text-lg rounded-2xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 w-full md:w-auto"
                  >
                    Bắt đầu tư vấn ngay
                  </button>
                </div>
                ...
                <div className="w-full md:w-1/2 relative">
                  <div className="relative aspect-[4/3] w-full">
                    ...
                    <div className="absolute -bottom-4 -left-4 w-32 h-32 bg-blue-100 rounded-full opacity-50 blur-2xl"></div>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-8 border border-cyan-100 shadow-[0_10px_40px_rgba(14,165,233,0.08)]">
                    ...
                    <div className="flex items-center justify-between mb-8">
                      <button 
                        ...
                        disabled={isSameWeek(currentWeek, startOfThisWeek, { weekStartsOn: 1 })} 
                      ...
                      <div className="text-lg font-semibold text-sky-900">
                        {(() => {
                          ...
                          const mondayMonth = formatInTimeZone(monday, 'Asia/Ho_Chi_Minh', 'MM/yyyy');
                          const sundayMonth = formatInTimeZone(sunday, 'Asia/Ho_Chi_Minh', 'MM/yyyy');
                          const isCurrentWeek = isSameWeek(currentWeek, startOfThisWeek, { weekStartsOn: 1 });
                          
                          const weekLabel = isCurrentWeek ? 'Tuần này' : 'Tuần sau';
                          
                          const dateRange = mondayMonth === sundayMonth
                            ? `${formatInTimeZone(monday, 'Asia/Ho_Chi_Minh', 'dd')} - ${formatInTimeZone(sunday, 'Asia/Ho_Chi_Minh', 'dd/MM/yyyy')}`
                            : `${formatInTimeZone(monday, 'Asia/Ho_Chi_Minh', 'dd/MM')} - ${formatInTimeZone(sunday, 'Asia/Ho_Chi_Minh', 'dd/MM/yyyy')}`;
                          
                          return (
                            <div className="flex flex-col items-center">
                              <span className={`text-sm font-medium mb-1 ${
                                isCurrentWeek ? 'text-cyan-600' : 'text-sky-500'
                              }`}>
                                {weekLabel}
                              </span>
                              <span>{dateRange}</span>
                      ...
                      <button 
                        className="w-14 h-14 rounded-2xl bg-white border border-sky-100 flex items-center justify-center text-sky-500 hover:text-sky-600 hover:border-sky-200 hover:bg-sky-50 transition-all disabled:opacity-30" 
                        disabled={isSameWeek(currentWeek, startOfNextWeek, { weekStartsOn: 1 })} 
